...
describe('challengifyReducer', () => {
  ...
  describe('TOGGLE_DAY', () => {
    it('sets the day of the specified challenge to ENABLED state', () => {
      const initialState = challengifyReducer({}, createChallenge('test'));

      expect(challengifyReducer(initialState, toggleDay(0, 0))[0].days[0]).toEqual('▣');
    });
  });
});
